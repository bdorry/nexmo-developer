...
Nexmo numbers that are linked to Nexmo applications will use the `answer_url` to retrieve an NCCO, and the `event_url` to send call status information to you. The optional `fallback_answer_url` can be configured in case `answer_url` or `event_url`, for events that are expected to deliver an NCCO, are offline, or return an HTTP error code.
